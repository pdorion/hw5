...
//https://api.weatherapi.com/v1/forecast.json?key=85c7eec3fadd4fe0ade12006213004&q=Chicago&days=3
...
window.addEventListener('DOMContentLoaded', async function() {
    ...
    getWeatherButton.addEventListener(`click`, async function(event){
      ...
      //get number of days to put into fetch
  
      // - Check to see if the user entered anything; if so:
      if (inputLocation.length > 0) {
  
        // - Construct a URL to call the WeatherAPI.com API
        let url = `https://api.weatherapi.com/v1/forecast.json?key=85c7eec3fadd4fe0ade12006213004&q=${inputLocation}&days=3`
        ...
        // for (i = 0, i < 3, i++) {
        //   let datee = json.forecast.forecastday[i].date
        //   console.log(date)
        // }
      }
      })
